...
```json
{
  "genres": [
    ...
    { "key": "subculture", "genreId": "101164", "genreLabel": "サブカル", "poolSize": 1, "trendingPoolSize": 1 }
  ],
  "period": "realtime",
  "historySize": 60,
  "referer": "https://x.com/",
...
- `genres`: 取得したいジャンルの配列。**1回の実行ごとに配列の各要素についてそれぞれ楽天ランキングAPIを叩き、投稿案をまとめてプールに追加する。** 要素を増減させれば好きなジャンルを何個でも組み合わせられる。
  ...
  - `genreId`: 楽天ジャンルID。`0` は総合(全ジャンル)。特定ジャンルに絞りたい場合は
    ...
    楽天市場のカテゴリページURL末尾の数字から調べて差し替える(例: 家電=100804、コスメ=100939、ホビー(フィギュア・プラモデル・トレーディングカードなどサブカル系)=101164 など。時期により変わるため要確認)。
  - `genreLabel`: 投稿文・ハッシュタグ・Webアプリのジャンルタブ/バッジに使う日本語ラベル。
  - `poolSize`: このジャンルで1回の実行あたり何件のランキング投稿案を新規追加するか。
  - `trendingPoolSize`: このジャンルで1回の実行あたり何件の急上昇商品を新規追加するか(`trendingEnabled: true` の場合)。
  - デフォルトでは「総合」に加えて「サブカル」(楽天のホビージャンル。フィギュア・プラモデル・トレカ・趣味コレクション等を含む)が設定済み。不要なら配列から削除、他のサブカル系ジャンル(同人・アニメ関連など)を追加したい場合は上記のジャンルID検索で調べて要素を追加する。
